...
export default function App() {
  ...
  async function fetchTodos() {
    ...
    try {
      const response = await fetch("https://clear-list-fsvt-gev69szr6-prakhar-deeps-projects.vercel.app//todos", {
        headers: {
  ...
  async function handleAuth(isRegister: boolean) {
    try {
      const response = await fetch(`https://clear-list-fsvt-gev69szr6-prakhar-deeps-projects.vercel.app//${isRegister ? 'register' : 'login'}`, {
        method: 'POST',
        headers: {
  ...
  async function deleteTodo(id: string) {
    if (!user) return;
    try {
      const response = await fetch(`https://clear-list-fsvt-gev69szr6-prakhar-deeps-projects.vercel.app//todos/${id}`, {
        method: "DELETE",
        headers: {
          'Authorization': `Bearer ${user.token}`
        }
